<?php
Style::begin("Search");
echo "&nbsp;-&nbsp;[<a href='" . URLROOT . "/adminsearch/advanced'>Reset</a>]</p>\n";
?>
    <div class="border ttborder">
	<form method="get" action="<?php echo URLROOT; ?>/adminsearch/advanced">
...
// Validates date in the form [yy]yy-mm-dd;
        ...
        if (count($_GET) > 0 && !$_GET['h']) {
            ...
            if ($ip) {
                ...
                if (!preg_match($regex, $ip)) {
                    Redirect::autolink(URLROOT."/adminsearch/advanced", "Bad IP.");
                }
                $mask = trim($_GET['ma']);
                if ($mask == "" || $mask == "255.255.255.255") {
                    ...
                    if (substr($mask, 0, 1) == "/") {
                        ...
                        if (!is_numeric($n) or $n < 0 or $n > 32) {
                            Redirect::autolink(URLROOT."/adminsearch/advanced", "Bad subnet mask.");
                        } else {
            ...
            if ($ratio) {
                if ($ratio == '---') {
                    ...
                    if (!is_numeric($ratio) || $ratio < 0) {
                        Redirect::autolink(URLROOT."/adminsearch/advanced", "Bad ratio.");
                    }
                    $where_is .= isset($where_is) ? " AND " : "";
                    ...
                    if ($ratiotype == "3") {
                        $ratio2 = trim($_GET['r2']);
                        if (!$ratio2) {
                            Redirect::autolink(URLROOT."/adminsearch/advanced", "Two ratios needed for this type of search.");
                        }
                        if (!is_numeric($ratio2) or $ratio2 < $ratio) {
                            Redirect::autolink(URLROOT."/adminsearch/advanced", "Bad second ratio.");
                        }
            ...
            if ($ul) {
                if (!is_numeric($ul) || $ul < 0) {
                    Redirect::autolink(URLROOT."/adminsearch/advanced", "Bad uploaded amount.");
                }
                $where_is .= isset($where_is) ? " AND " : "";
                $where_is .= " u.uploaded ";
                $ultype = $_GET['ult'];
                ...
                if ($ultype == "3") {
                    $ul2 = trim($_GET['ul2']);
                    if (!$ul2) {
                        Redirect::autolink(URLROOT."/adminsearch/advanced", "Two uploaded amounts needed for this type of search.");
                    }
                    if (!is_numeric($ul2) or $ul2 < $ul) {
                        Redirect::autolink(URLROOT."/adminsearch/advanced", "Bad second uploaded amount.");
                    }
            ...
            if ($dl) {
                if (!is_numeric($dl) || $dl < 0) {
                    Redirect::autolink(URLROOT."/adminsearch/advanced", "Bad downloaded amount.");
                }
                $where_is .= isset($where_is) ? " AND " : "";
                ...
                if ($dltype == "3") {
                    $dl2 = trim($_GET['dl2']);
                    if (!$dl2) {
                        Redirect::autolink(URLROOT."/adminsearch/advanced", "Two downloaded amounts needed for this type of search.");
                    }
                    if (!is_numeric($dl2) or $dl2 < $dl) {
                        Redirect::autolink(URLROOT."/adminsearch/advanced", "Bad second downloaded amount.");
                    }
            ...
            if ($date) {
                if (!$date = mkdate($date)) {
                    Redirect::autolink(URLROOT."/adminsearch/advanced", "Invalid date.");
                }
                $q .= ($q ? "&amp;" : "") . "d=$date";
                $datetype = $_GET['dt'];
                $q .= ($q ? "&amp;" : "") . "dt=$datetype";
                if ($datetype == "0") {
                    ...
                    if ($datetype == "3") {
                        ...
                        if ($date2) {
                            ...
                            Redirect::autolink(URLROOT."/adminsearch/advanced", "Two dates needed for this type of search.");
                        }
                    } elseif ($datetype == "1") {
                        $where_is .= "< '$date'";
                    } elseif ($datetype == "2") {
                ...
                if ($lasttype == "0") {
                    ...
                    if ($lasttype == "3") {
                        ...
                        if ($last2) {
                            ...
                            Redirect::autolink(URLROOT."/adminsearch/advanced", "The second date is not valid.");
                        }
                    } elseif ($lasttype == "1") {
                        $where_is .= "< '$last'";
                    } elseif ($lasttype == "2") {
		...
		u.class, u.uploaded, u.downloaded, u.donated, u.modcomment, u.enabled, u.warned, u.invited_by";
            ...
            $q = isset($q) ? ($q . "&amp;") : "";
            $perpage = 25;
            list($pagerbuttons, $limit) = Pagination::pager($perpage, $count, "".URLROOT."/adminsearch/advanced?$q");
            $query .= $limit;
            $res = DB::run($query, $params)->fetchAll();

            if (!$res) {
                Redirect::to(URLROOT."/adminsearch/advanced", "No user was found.");
            } else {
                if ($count > $perpage) {
                    echo $pagerbuttons;
                }
                echo "<br><form action='".URLROOT."/adminsearch/advanced?do=warndisable' method='post'>";
